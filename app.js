  (function () {
    ...
    function playFx(name, seconds) {
      ...
      stopFx();
      const token = fxToken;
      const a = new Audio("./audio/fx/" + name + ".wav?v=2");
      fxAudio = a;
      a.volume = 0.9;
    ...
    function paintRetryBadge(el, n, extraClass) {
      if (!el) return;
      ...
      if (!badge) {
        ...
        badge.className = "nav-badge" + (extraClass ? " " + extraClass : "");
    ...
    function syncRetryBadge(store) {
      ...
      const nav = $("navRecords");
      if (nav) {
        paintRetryBadge(nav, n);
        nav.setAttribute("aria-label", n ? "Records，" + n + " 道错题" : "Records");
      }
      const retryBtn = $("btnRetryReview");
      if (retryBtn) {
        paintRetryBadge(retryBtn, n, "rec-badge");
    ...
    function renderRecords() {
      ...
      if ($("recordsActions")) {
        ...
        retryBtn.id = "btnRetryReview";
        retryBtn.className = "rec-action has-badge";
        retryBtn.innerHTML =
        ...
        challengeBtn.innerHTML = "<strong>难度挑战</strong><span>加长听力 · 造句 · 口语</span>";
    ...
    function renderDoneSummary(settle, ok, total, egg) {
      ...
      earned.innerHTML =
        ...
        (settle.double ? '<div class="earn-row"><span>双倍券</span><strong>已翻倍</strong></div>' : "") +
    ...
    function startDonePractice(egg) {
      ...
      const tip = document.createElement("p");
      tip.className = "follow-tip";
      tip.textContent = "先听翻翻龟，再大声跟读 3 遍。不用录音。";
      box.appendChild(tip);
      ...
      btn.addEventListener("click", () => {
        ...
        pathBtn.type = "button";
        pathBtn.className = "btn-start";
        pathBtn.textContent = "看学期路径";
    ...
    function init() {
      ...
      function playHomeFile(who) {
        return new Promise((resolve) => {
          if (!$("screenHome") || $("screenHome").classList.contains("hidden")) { resolve(false); return; }
          const src = homeClips[who];
          if (!src) { resolve(false); return; }
          const a = new Audio(src + "?v=4");
          homeAudio = a;
          let settled = false;
          const finish = (ok) => {
            if (settled) return;
          ...
          const started = a.play();
          if (started && started.then) started.then(() => {}).catch(() => finish(false));
        });
      }
      ...
      function playHomeSlogans(force) {
        ...
        if (!V || !V.isEnabled()) return;
        if (!$("screenHome") || $("screenHome").classList.contains("hidden")) return;
        if (force) {
          stopHomeAudio();
          homeSloganBusy = false;
        }
      ...
      function onUserGesture() {
        if (!audioPrimed && V && V.prime) {
          V.prime().then((ok) => { if (ok) audioPrimed = true; });
        }
      ...
      window.addEventListener("pageshow", () => {
        if ($("screenHome") && !$("screenHome").classList.contains("hidden")) playHomeSlogans();
      });
      if ($("btnHomeSlogan")) {
